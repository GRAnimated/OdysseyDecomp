...
NERVE_MAKE(PlayGuideCamera, Wait);
...
PlayGuideCamera::PlayGuideCamera(const char* name, const al::LayoutInitInfo& info,
    ...
    : al::LayoutActor(name), field_130(actor) {
...
void PlayGuideCamera::start() {
    if (!rs::isActiveDemo(field_130))
        al::setNerve(this, &NrvPlayGuideCamera.Appear);
}

void PlayGuideCamera::exeHide() {
    if (!field_129 && !rs::isActiveDemo(field_130) &&
        al::isExistCameraInputAtDisableTiming(this, 0) &&
        !rs::isPlayerHackGroupUseCameraStick(field_130)) {
        al::setNerve(this, &NrvPlayGuideCamera.Appear);
        return;
    }
...
bool PlayGuideCamera::tryAppear() {
    if (!rs::isActiveDemo(field_130) && al::isExistCameraInputAtDisableTiming(this, 0) &&
        !rs::isPlayerHackGroupUseCameraStick(field_130)) {
        al::setNerve(this, &NrvPlayGuideCamera.Appear);
        return true;
    }
