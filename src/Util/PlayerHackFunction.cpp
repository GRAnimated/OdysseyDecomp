...
static void initHackEndParam(HackEndParam* hackEndParam, PlayerHackKeeper* hackKeeper) {
    ...
    if (hackKeeper->isCancellingHack()) {
        ...
        hackEndParam->dir.set(hackFront);
        al::makeQuatFrontUp(&hackEndParam->quat, hackFront, hackUp);
    }
    ...
    hackEndParam->delayFrames = 0;
    hackEndParam->isDamage = false;
    hackEndParam->hasVelocity = false;
}
...
void endHackTargetQuat(IUsePlayerHack** playerHack, const sead::Quatf& quat,
                       const sead::Vector3f& dir) {
    PlayerHackKeeper* hackKeeper = (*playerHack)->getPlayerHackKeeper();
    HackEndParam hackEndParam;

    initHackEndParam(&hackEndParam, hackKeeper);
    hackEndParam.quat = quat;
    hackEndParam.dir.set(dir);
...
void endHackFromTargetPos(IUsePlayerHack** playerHack, const sead::Vector3f& targetPos,
                          const sead::Quatf& quat, const sead::Vector3f& dir) {
    PlayerHackKeeper* hackKeeper = (*playerHack)->getPlayerHackKeeper();
    HackEndParam hackEndParam;
...
void endHackDirEscapeScalePose(IUsePlayerHack** playerHack, const sead::Quatf& quat,
...
void endHackAirVelocity(IUsePlayerHack** playerHack, const sead::Vector3f& targetPos,
                        const sead::Quatf& quat, const sead::Vector3f& vel, s32 delayFrames) {
    PlayerHackKeeper* hackKeeper = (*playerHack)->getPlayerHackKeeper();
    HackEndParam hackEndParam;
    ...
    hackEndParam.quat = quat;
    hackEndParam.delayFrames = delayFrames;
...
inline s32 getSegment(f32 target, f32 currentSegment, bool lookForward) {
    f32 step = 0.0f;
    for (s32 i = 1; i < 600; i++) {
        f32 next = currentSegment + step;
        if (al::isInRange(target, currentSegment, next))
            return i;

        step = sead::Mathf::clamp(step + ((lookForward) ? 1.0f : -1.0f), -35.0f, 35.0f);
        currentSegment = next;
    }
    return 600;
}
...
// misplaced ldr and stack too small
void endHackBazookaElectricToDestination(IUsePlayerHack** playerHack, const sead::Vector3f& start,
                                         const sead::Vector3f& end, const sead::Quatf& quat,
                                         s32 delayFrames, f32 height) {
    sead::Vector3f startCpy = start;
    PlayerHackKeeper* hackKeeper = (*playerHack)->getPlayerHackKeeper();
    HackEndParam hackEndParam;
    ...
    if (startCpy.y < end.y)
        maxHeight += (end.y - startCpy.y);
    path.initFromUpVector(startCpy, end, sead::Vector3f::ey, maxHeight);

    s32 stepsAscend = getSegment(maxHeight, 0.0f, true);
    s32 stepsDescend = getSegment(end.y, maxHeight + startCpy.y + 0.0f, false);

    f32 rate = 1.0f / (stepsDescend + stepsAscend);
    f32 vely = stepsAscend;
    hackEndParam.vel = rate * (end - startCpy);
    hackEndParam.vel.y = vely;

    hackEndParam.targetPos = startCpy;
    hackEndParam.quat = quat;
...
bool requestDamage(IUsePlayerHack* playerHack) {
    if (playerHack->getPlayerHackKeeper()->getDamageKeeper()->isDamageInvalid())
        return false;
...
bool isDamageVisibilityHide(const IUsePlayerHack* playerHack) {
    return playerHack->getPlayerHackKeeper()->getDamageKeeper()->isDamageInvalid() &&
...
bool isHackerStopMove(const al::LiveActor* actor, const IUsePlayerHack* playerHack,
    ...
    if (playerHack->getPlayerHackKeeper()->getInput()->isMove() ||
        al::calcSpeedH(actor) >= speedLimit) {
        return false;
    }
...
bool isHackerStopMoveGround(const al::LiveActor* actor, const IUsePlayerHack* playerHack,
    ...
    if (playerCollider) {
        if (isCollidedGround(playerCollider)) {
            return !(al::calcSpeedExceptDir(actor, getCollidedGroundNormal(playerCollider)) >=
                     speedLimit);
        }

        return isHackerStopMove(actor, playerHack, speedLimit);
    }

    if (!isCollidedGround(actor))
        return isHackerStopMove(actor, playerHack, speedLimit);
    return !(al::calcSpeedExceptDir(actor, getCollidedGroundNormal(actor)) >= speedLimit);
}
...
bool sendMsgHackerNoReactionWithoutShine(const IUsePlayerHack* playerHack, al::HitSensor* receiver,
                                         al::HitSensor* sender) {
    return al::sendMsgPlayerItemGet(receiver, sender);
}
...
// TODO: rename `param3`, `param4` here and in header
...
// TODO: rename `param2`, `param3`, `param4` here and in header
